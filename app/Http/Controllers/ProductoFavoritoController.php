...
class ProductoFavoritoController extends Controller
{
    ...
    public function index()
    {
        if(Auth::guest()) {
            return redirect()->route('web.login')
                ->with('mensaje', 'Debes Iniciar Sesion para ver tus Productos Favoritos')
                ->with('icono', 'warning');
        }
        $ajuste = Ajuste::first();
    ...
    public function store(Request $request)
    {
        ...
        if($productoFavorito = ProductoFavorito::where('usuario_id', Auth::user()->id)
            ...
            return redirect()->back()
                ->with('mensaje','El producto ya está en tus favoritos')
                ->with('icono', 'info');
        }
        ...
        return redirect()->back()
            ->with('mensaje','Producto Favorito Agregado')
            ->with('icono', 'success'); 
    }
    ...
    public function destroy($id)
    {
        ...
        if(!$productoFavorito) {
            return redirect()->back()
                ->with('mensaje','Producto Favorito no encontrado')
                ->with('icono', 'error');
        }
